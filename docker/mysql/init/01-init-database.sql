...
CREATE TABLE IF NOT EXISTS `board_categories` (
  ...
  `name` VARCHAR(50) NOT NULL UNIQUE COMMENT '게시판 카테고리명',
...
CREATE TABLE IF NOT EXISTS `posts` (
  ...
  `category_id` BIGINT NOT NULL COMMENT '게시판 카테고리 ID',
  `user_id` BIGINT NOT NULL COMMENT '작성자 ID',
  `product_id` BIGINT NULL COMMENT '연관 상품 ID',
  ...
  `is_answered` BOOLEAN DEFAULT FALSE COMMENT '답변 완료 여부',
...
CREATE TABLE IF NOT EXISTS `comments` (
  ...
  `user_id` BIGINT NOT NULL COMMENT '작성자 ID',
  `parent_id` BIGINT NULL COMMENT '상위 댓글 ID',
...
-- 게시판 카테고리 초기 데이터
INSERT INTO `board_categories` (`name`, `description`, `sort_order`, `admin_only`) VALUES
('공지사항', '중요한 공지사항을 게시합니다', 1, TRUE),
('자유게시판', '자유롭게 의견을 나누는 공간입니다', 2, FALSE),
('상품문의', '상품에 대한 문의를 남겨주세요', 3, FALSE),
('리뷰', '구매 후기를 공유해주세요', 4, FALSE),
('이벤트', '진행 중인 이벤트를 확인하세요', 5, TRUE);

-- 초기화 완료 확인
SELECT 'POOROOM 쇼핑몰 데이터베이스 초기화 완료!' as status;